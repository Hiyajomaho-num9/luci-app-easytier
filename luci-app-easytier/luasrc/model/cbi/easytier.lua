...
local ui_mode = uci:get("easytier", "easytier", "ui_mode") or "advanced"

s:tab("general", translate("General Settings"))
if ui_mode == "advanced" then
...
mode_switch.inputtitle = translate("Switch to Simple Mode")
mode_switch.description = translate("Click to switch between Simple and Advanced mode")
mode_switch.inputstyle = "apply"
mode_switch.write = function()
    local uci = require "luci.model.uci".cursor()
    local current_mode = uci:get("easytier", "easytier", "ui_mode") or "advanced"
    local new_mode = (current_mode == "simple") and "advanced" or "simple"
    uci:set("easytier", "easytier", "ui_mode", new_mode)
    uci:commit("easytier")
    luci.http.redirect(luci.dispatcher.build_url("admin/services/easytier"))
end

-- 更新按钮标题根据当前模式
if ui_mode == "simple" then
    mode_switch.inputtitle = translate("Switch to Advanced Mode")
else
    mode_switch.inputtitle = translate("Switch to Simple Mode")
end
...
if ui_mode == "advanced" then
    ...
    -- Simple模式下默认使用etcmd模式，隐藏选择
    etcmd = s:taboption("general", Value, "etcmd", translate("Startup Method"))
    etcmd.default = "etcmd"
    etcmd.template = "cbi/dvalue"
    etcmd.value = "etcmd"
    -- 在simple模式下强制设置为etcmd
    local uci_simple = require "luci.model.uci".cursor()
    uci_simple:set("easytier", "easytier", "etcmd", "etcmd")
    uci_simple:commit("easytier")
end
